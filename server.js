          ...
          if (module.files.video) {
            ...
            if (!module.files.video.url) {
              if (module.files.video.filename) {
                module.files.video.url = `${req.protocol}://${req.get('host')}/uploads/${module.files.video.filename}`;
              } else {
                ...
                try {
                  ...
                  if (fs.existsSync(uploadsDir)) {
                    ...
                    if (videoFiles.length > 0) {
                      module.files.video.url = `${req.protocol}://${req.get('host')}/uploads/${videoFiles[0]}`;
                    }
                  }
                } catch (error) {
          ...
          if (module.files.pdf) {
            ...
            if (!module.files.pdf.url) {
              if (module.files.pdf.filename) {
                module.files.pdf.url = `${req.protocol}://${req.get('host')}/uploads/${module.files.pdf.filename}`;
              } else {
                // For existing data without filename, use a default approach
                try {
                  const uploadsDir = path.join(__dirname, 'uploads');
                  if (fs.existsSync(uploadsDir)) {
                    ...
                    if (pdfFiles.length > 0) {
                      module.files.pdf.url = `${req.protocol}://${req.get('host')}/uploads/${pdfFiles[0]}`;
                    }
                  }
                } catch (error) {
                  console.error('Error reading uploads directory:', error);
